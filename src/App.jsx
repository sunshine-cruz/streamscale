...
function Home() {
  const [page, setPage] = useState("dashboard"), [selected, setSelected] = useState(videos[0]), [query, setQuery] = useState(""), [genre, setGenre] = useState("All genres"), [publisher, setPublisher] = useState("All publishers"), [role, setRole] = useState("guest"), [comments, setComments] = useState(initialComments), [comment, setComment] = useState(""), [rating, setRating] = useState(0), [notice, setNotice] = useState("");
  const filtered = useMemo(() => videos.filter((v) => (v.title.toLowerCase().includes(query.toLowerCase()) || v.publisher.toLowerCase().includes(query.toLowerCase())) && (genre === "All genres" || v.genre === genre) && (publisher === "All publishers" || v.publisher === publisher)), [query, genre, publisher]);
  ...
  }}><Icon name="logout" /></button></>}</div></header>{notice && <div className="notice" role="status"><span>{notice}</span><button onClick={() => setNotice("")} aria-label="Dismiss">×</button></div>}<main>{page === "dashboard" && <Dashboard openVideo={openVideo} goSearch={() => navigate("search")} />} {page === "search" && <SearchPage query={query} setQuery={setQuery} genre={genre} setGenre={setGenre} publisher={publisher} setPublisher={setPublisher} results={filtered} openVideo={openVideo} />} {page === "details" && <Details video={selected} back={() => setPage("dashboard")} role={role} comments={comments} comment={comment} setComment={setComment} addComment={addComment} rating={rating} setRating={setRating} setNotice={setNotice} />} {page === "upload" && <UploadPage setNotice={setNotice} goDashboard={() => setPage("dashboard")} />}</main><Footer navigate={navigate} /></div>;
}
function Dashboard({ openVideo, goSearch }) {
...
function SearchPage({ query, setQuery, genre, setGenre, publisher, setPublisher, results, openVideo }) {
...
function UploadPage({ setNotice, goDashboard }) {
